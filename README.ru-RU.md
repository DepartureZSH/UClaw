...
<a href="#почему-uclaw">Почему UClaw</a> •
...
**UClaw** — это мост между мощными AI-агентами и повседневными пользователями. Построенный на базе [OpenClaw](https://github.com/OpenClaw), он превращает управление AI через командную строку в доступный и красивый десктоп-опыт — терминал не нужен.

Автоматизация рабочих процессов, управление AI-каналами или планирование интеллектуальных задач — UClaw предоставляет интерфейс для эффективного использования AI-агентов.

UClaw поставляется с предустановленными лучшими практиками для провайдеров моделей и нативно поддерживает Windows, а также многоязычные настройки. Вы можете тонко настроить расширенные параметры через **Настройки → Дополнительно → Режим разработчика**.
...
Руководство для обычных пользователей находится в [user-guide.md](user-guide.md), а офлайн HTML-версия — в [docs/user-manual.html](docs/user-manual.html). В нём описаны поля страниц, скриншоты, детали настройки каналов и типовые шаги диагностики.

---

## Почему UClaw

Создание AI-агентов не должно требовать владения командной строкой. Философия UClaw проста: **мощные технологии заслуживают интерфейса, который уважает ваше время.**

| Проблема | Решение UClaw |
|----------|---------------|
...
UClaw построен непосредственно на официальном ядре **OpenClaw**. Вместо отдельной установки мы встраиваем среду выполнения в приложение для бесшовного опыта "всё включено".
...
При выборе другого агента через `@agent` UClaw переключается непосредственно в контекст этого агента вместо ретрансляции через агента по умолчанию. Рабочие пространства агентов по умолчанию разделены, но более строгая изоляция зависит от настроек песочницы OpenClaw.
...
Для пользовательских идентификаторов учётных записей каналов UClaw требует совместимый с OpenClaw канонический формат (`[a-z0-9_-]`, строчные буквы, максимум 64 символа, должен начинаться с буквы или цифры) для предотвращения ошибок маршрутизации.
UClaw также включает официальный плагин личного WeChat от Tencent, позволяя подключить WeChat напрямую со страницы Каналы через встроенный QR-код.
...
Когда совместимый шлюз отклоняет `/models` по причинам, не связанным с аутентификацией, UClaw автоматически переключается на легковесный зонд `/chat/completions` или `/responses` при проверке API-ключа.

### 🌙 Адаптивные темы
Светлая тема, тёмная тема или синхронизация с системой. UClaw автоматически адаптируется к вашим предпочтениям.

### 🚀 Управление автозапуском
В **Настройки → Общие** вы можете включить **Запускать при старте системы**, чтобы UClaw автоматически запускался после входа в систему.
...
Скачайте последний релиз для вашей платформы со страницы [Releases](https://github.com/DepartureZSH/UClaw/releases).
...
При первом запуске UClaw **Мастер настройки** проведёт вас через:

1. **Язык и регион** — настройка предпочтительного языка и региона
2. **Workspace** — выбор места хранения данных OpenClaw
3. **Проверка окружения** — проверка встроенной среды выполнения и Gateway
4. **AI-настройка** — настройка New API endpoint и модели по умолчанию при необходимости
5. **Необходимые компоненты** — установка локальных инструментов по умолчанию

Мастер предварительно выбирает системный язык, если он поддерживается, иначе переключается на английский.
Если выбранный workspace уже содержит `.openclaw/openclaw.json`, мастер использует существующую конфигурацию OpenClaw и пропускает ввод AI-ключа.

> Примечание о web search: по умолчанию UClaw использует web search в стиле Kimi/Moonshot через сервис New API. Обычным пользователям достаточно использовать встроенную настройку или переключение в один клик; разворачивать собственный endpoint или вручную редактировать конфигурацию OpenClaw не нужно.

### Настройки прокси

UClaw включает встроенные настройки прокси для сред, где Electron, шлюз OpenClaw или каналы вроде Telegram должны выходить в интернет через локальный прокси-клиент.
...
- Если расширенные поля прокси пусты, UClaw использует `Прокси-сервер`.
- Сохранение настроек прокси немедленно повторно применяет сеть Electron и автоматически перезапускает шлюз.
- UClaw также синхронизирует прокси с конфигурацией канала Telegram в OpenClaw, когда Telegram включён.
- При перезапуске шлюза существующий прокси канала Telegram сохраняется, если прокси UClaw отключен.
...
### Краткая справка по полям страниц

| Страница | Основные поля и кнопки | Частые действия |
|----------|------------------------|-----------------|
| Chat | Новый чат, список сессий, текущая цель, обновить, поле ввода, вложения, отправить/остановить, статус Gateway | Задавать вопросы, просить web search с источниками, проверять подключение текущего Agent |
| Models | AI-провайдеры моделей, model ID, получить список моделей, статус API-ключа, секция web-search | Проверить New API, обновить доступные модели, проверить поисковую модель |
| Settings | Тема, язык, автозапуск, статус Gateway, порт, логи, прокси, обновления, о приложении | Настроить внешний вид и язык, проверить обновления, открыть логи, настроить прокси |
| Channels | Добавить канал, account ID, аккаунт по умолчанию, привязка Agent, диагностика, перезапуск Gateway | Подключить WeChat/Feishu/DingTalk и выбрать Agent для ответов |
| Cron | Название задачи, время запуска, prompt, Agent, аккаунт отправителя, получатель, состояние включения | Создавать ежедневные отчёты, периодические сводки, поиски или внешнюю доставку |
| Skills | Название навыка, состояние, путь источника, настройки/API key | Проверять встроенные навыки, открывать папки и вводить настройки |

### Поля настройки каналов

UClaw поддерживает несколько аккаунтов для одного канала. Account ID, введённый вручную, должен состоять из строчных букв, цифр, подчёркиваний или дефисов, быть не длиннее 64 символов и начинаться с буквы или цифры. QR-каналы вроде WeChat и WhatsApp управляются плагином и обычно не требуют ручного ввода секретов.

| Канал | Способ подключения | Обязательные поля |
|-------|--------------------|-------------------|
| WeChat | QR-код | Поля не нужны; следуйте QR-подтверждению |
| WhatsApp | QR-код | Поля не нужны; сканируйте телефоном |
| Telegram | Bot token | Bot token, разрешённые user ID |
| Discord | Bot token | Bot token, guild ID, channel ID (необязательно) |
| Feishu / Lark | Учётные данные приложения | App ID, app secret |
| DingTalk | Учётные данные приложения | Client ID / AppKey, Client Secret / AppSecret |
| WeCom | Учётные данные приложения | Bot ID, app secret |
| QQ Bot | Учётные данные приложения | App ID, client secret |

---

## Архитектура

UClaw использует **двухпроцессную архитектуру с унифицированным уровнем Host API**. Рендерер обращается к единой абстракции клиента, а Electron Main управляет выбором протокола и жизненным циклом процессов:

```
┌─────────────────────────────────────────────────────────────────┐
│                     Десктоп-приложение UClaw                    │
│                                                                 │
│  ┌────────────────────────────────────────────────────────────┐ │
...
- UClaw — это приложение Electron, поэтому **один экземпляр приложения обычно отображается как несколько процессов ОС** (main/renderer/zygote/utility). Это нормально.
...
- Нажатие кнопки закрытия окна (`X`) скрывает UClaw в трей; это **не** полностью закрывает приложение. Используйте меню трея **Quit UClaw** для полного завершения.
...
- использует временный каталог `userData` UClaw
...
Мы приветствуем вклад сообщества! Исправления багов, новые функции, улучшения документации или переводы — каждый вклад делает UClaw лучше.
...
UClaw построен на плечах отличных проектов с открытым исходным кодом:

- [ClawX](https://github.com/ValueCell-ai/ClawX) – исходная линия open-source десктоп-приложения от ValueCell-ai
...
### Партнёрская программа UClaw 🚀

Мы запускаем Партнёрскую программу UClaw и ищем партнёров, которые могут помочь представить UClaw большему числу клиентов, особенно тем, у кого есть потребности в кастомных AI-агентах или автоматизации.

Партнёры помогают связывать нас с потенциальными пользователями и проектами, а команда UClaw предоставляет полную техническую поддержку, кастомизацию и интеграцию.

Если вы работаете с клиентами, заинтересованными в AI-инструментах или автоматизации, мы будем рады сотрудничеству.

Для сообщений об ошибках, вопросов по релизам и воспроизводимых заметок по диагностике используйте [GitHub Issues](https://github.com/DepartureZSH/UClaw/issues).
...
UClaw выпускается под [лицензией MIT](LICENSE). Вы можете свободно использовать, модифицировать и распространять это программное обеспечение.

---

<p align="center">
<sub>Поддерживается DepartureZSH</sub>
</p>
