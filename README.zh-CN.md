...
<p align="center">
  <a href="#功能特性">功能特性</a> •
  <a href="#为什么选择-uclaw">为什么选择 UClaw</a> •
  <a href="#快速上手">快速上手</a> •
  <a href="#系统架构">系统架构</a> •
  <a href="#开发指南">开发指南</a> •
  <a href="#参与贡献">参与贡献</a>
...
**UClaw** 是连接强大 AI 智能体与普通用户之间的桥梁。基于 [OpenClaw](https://github.com/OpenClaw) 构建，它将命令行式的 AI 编排转变为易用、美观的桌面体验——无需使用终端。

无论是自动化工作流、连接通讯软件，还是调度智能定时任务，UClaw 都能提供高效易用的图形界面，帮助你充分发挥 AI 智能体的能力。

UClaw 预置了最佳实践的模型供应商配置，原生支持 Windows 平台以及多语言设置。当然，你也可以通过 **设置 → 高级 → 开发者模式** 来进行精细的高级配置。
...
完整普通用户手册见 [user-guide.md](user-guide.md)，可离线打开的网页手册见 [docs/user-manual.html](docs/user-manual.html)。手册中补充了对话页、模型页、设置页、频道页、技能页和定时任务页的字段说明、截图和常见配置步骤。

---

## 为什么选择 UClaw

构建 AI 智能体不应该需要精通命令行。UClaw 的设计理念很简单：**强大的技术值得拥有一个尊重用户时间的界面。**

| 痛点 | UClaw 解决方案 |
|------|----------------|
| 复杂的命令行配置 | 一键安装，配合启动页和公司密钥配置 |
...
UClaw 直接基于官方 **OpenClaw** 核心构建。无需单独安装，我们将运行时嵌入应用内部，提供开箱即用的无缝体验。
...
当你使用 `@agent` 选择其他智能体时，UClaw 会直接切换到该智能体自己的对话上下文，而不是经过默认智能体转发。各 Agent 工作区默认彼此分离，但更强的运行时隔离仍取决于 OpenClaw 的 sandbox 配置。
...
对于自定义频道账号 ID，UClaw 现在会强制校验 OpenClaw 兼容的规范格式（`[a-z0-9_-]`、小写、最长 64 位、且必须以字母或数字开头），避免路由匹配异常。
UClaw 现在还内置了腾讯官方个人微信渠道插件，可直接在 Channels 页面通过内置二维码流程完成微信连接。
...
UClaw 还会内置预装完整的文档处理技能（`pdf`、`xlsx`、`docx`、`pptx`），在启动时自动部署到托管技能目录（默认 `~/.openclaw/skills`），并在首次安装时默认启用。额外预装技能（`find-skills`、`self-improving-agent`、`tavily-search`、`brave-web-search`）也会默认启用；若缺少必需的 API Key，OpenClaw 会在运行时给出配置错误提示。
...
如果兼容网关的 `/models` 因非鉴权原因不可用，UClaw 会在校验 API Key 时自动降级为轻量的 `/chat/completions` 或 `/responses` 探测。

### 🌙 自适应主题
支持浅色模式、深色模式或跟随系统主题。UClaw 自动适应你的偏好设置。
...
- ZIP 便携版不使用应用内自动安装更新。请下载新版 ZIP，退出 UClaw 后替换程序文件；内置 OpenClaw 运行时会随 UClaw 一起更新。
- AI 与联网搜索配置由公司配置服务自动下发。
- 如果随包凭证缺失或过期，UClaw 会显示 **公司密钥** 页面。普通用户只需要输入运维提供的公司密钥，不需要选择工作目录，也不需要手动填写 API 地址或 API Key。
- 旧 `/setup` 入口会打开公司密钥页面，仅用于制作人员初始化或重新绑定随盘工作台；工作区设置向导不属于商业 ZIP 的普通用户流程。

> 联网搜索说明：UClaw 默认使用 New API 提供的 Kimi/Moonshot 风格联网搜索能力。普通用户只需要使用应用内的一键配置或一键切换，不需要自行部署接口，也不需要手动编辑 OpenClaw 配置文件。

### 代理设置

UClaw 内置了代理设置，适用于需要通过本地代理客户端访问外网的场景，包括 Electron 本身、OpenClaw Gateway，以及 Telegram 这类频道的联网请求。
...
- 如果启用了 Telegram，UClaw 还会把代理同步到 OpenClaw 的 Telegram 频道配置中。
- 当 UClaw 代理处于关闭状态时，Gateway 的常规重启会保留已有的 Telegram 频道代理配置。
...
| 页面 | 主要字段和按钮 | 普通用户常用操作 |
|------|----------------|------------------|
| 对话 | 新对话、会话列表、当前对话对象、刷新、输入框、附件、发送/停止、网关状态 | 提问、要求联网搜索、查看当前 Agent 是否连接 |
| 模型 | AI 模型提供商、模型 ID、获取模型列表、API 密钥状态、联网搜索 web-search | 检查 New API 是否已配置，刷新模型列表，确认搜索模型可用 |
| 设置 | 主题、语言、开机自启、网关状态、端口、日志、代理、更新、关于 | 调整外观和语言，检查更新，打开日志，配置代理 |
| 频道 | 添加频道、账号 ID、默认账号、绑定 Agent、诊断、重启网关 | 连接微信/飞书/钉钉等外部消息入口，并指定由哪个 Agent 回复 |
| 定时任务 | 任务名称、触发时间、提示词、执行 Agent、发送账号、接收目标、启停状态 | 创建日报、定时总结、定期搜索或外部消息投递 |
| 技能 | 技能名称、启用状态、来源位置、配置/API Key | 查看内置技能是否启用，按需要打开技能目录或填写技能配置 |

### 频道配置字段

UClaw 的频道页支持多账号。手动填写账号 ID 时只能使用小写字母、数字、下划线和短横线，最长 64 位，并且必须以字母或数字开头。微信和 WhatsApp 这类扫码登录频道会由插件流程管理账号，不需要普通用户手写凭证。

| 频道 | 连接方式 | 需要填写的字段 |
|------|----------|----------------|
| 微信 WeChat | 扫码 | 不需要手写字段，按二维码流程确认即可 |
| WhatsApp | 扫码 | 不需要手写字段，使用手机 WhatsApp 扫码 |
| Telegram | Bot Token | 机器人令牌、允许的用户 ID |
| Discord | Bot Token | 机器人令牌、服务器 ID、频道 ID（可选） |
| 飞书 Feishu / Lark | 应用凭证 | 应用 ID、应用密钥 |
| 钉钉 DingTalk | 应用凭证 | Client ID / AppKey、Client Secret / AppSecret |
| 企业微信 WeCom | 应用凭证 | 机器人 Bot ID、应用 Secret |
| QQ Bot | 应用凭证 | App ID、Client Secret |

---

## 系统架构

UClaw 采用 **双进程 + Host API 统一接入架构**。渲染进程只调用统一客户端抽象，协议选择与进程生命周期由 Electron 主进程统一管理：
...
- UClaw 基于 Electron，**单个应用实例出现多个系统进程是正常现象**（main/renderer/zygote/utility）。
...
- 点击窗口关闭按钮（`X`）默认只是最小化到托盘，并不会完全退出应用。请在托盘菜单中选择 **Quit UClaw** 执行完整退出。
...
我们欢迎社区的各种贡献！无论是修复 Bug、开发新功能、改进文档还是翻译——每一份贡献都让 UClaw 变得更好。
...
UClaw 构建于以下优秀的开源项目之上：

- [ClawX](https://github.com/ValueCell-ai/ClawX) – 原始开源桌面应用谱系，由 ValueCell-ai 发起
...
我们正在启动 UClaw 合作伙伴计划，寻找能够帮助我们将 UClaw 介绍给更多客户的合作伙伴，尤其是那些有定制化 AI 智能体或自动化需求的客户。

合作伙伴负责帮助我们连接潜在用户和项目，UClaw 团队则提供完整的技术支持、定制开发与集成服务。

如果你服务的客户对 AI 工具或自动化方案感兴趣，欢迎与我们合作。

欢迎通过 [GitHub Issues](https://github.com/DepartureZSH/UClaw/issues) 反馈问题、提出建议或交流合作。
...
<p align="center">
  <img src="https://api.star-history.com/svg?repos=DepartureZSH/UClaw&type=Date" alt="Stars 历史图表" />
</p>
...
UClaw 基于 [MIT 许可证](LICENSE) 发布。你可以自由地使用、修改和分发本软件。

---

<p align="center">
  <sub>由 DepartureZSH 维护</sub>
</p>
