...
遇到启动或运行错误时，点击“复制诊断信息”会复制脱敏诊断内容；点击“导出诊断包”会把同类内容保存到 `data/uclaw/diagnostics/` 并打开所在文件夹。诊断包包含版本、平台、数据目录、启动阶段、Gateway/频道状态和最近日志摘要，不包含 API Key、公司密钥、token、cookie 或 OAuth 凭据。
