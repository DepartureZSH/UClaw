...
!macro customCheckAppRunning
  ...
  ${if} $R0 == 0
    ...
    # This covers UClaw.exe (multiple Electron processes), openclaw-gateway.exe,
  ...
  ; Even if UClaw.exe was not detected as running, orphan child processes
  ...
  ; PowerShell path-based kill may miss processes if the old UClaw was installed
  ...
  ; "Can't modify UClaw's files" retry loop -> "UClaw 无法关闭" dialog.
  ;
  ...
  ; files (antivirus, indexing) causing a blocking "UClaw 无法关闭" dialog.
...
; "UClaw 无法关闭" retry dialog when the old uninstaller fails.
;
...
!macro customUnInstall
  ...
  _cu_removeData:
    ...
    RMDir /r "$LOCALAPPDATA\uclaw"
    RMDir /r "$APPDATA\uclaw"

    ; --- Retry: if directories still exist (locked files), wait and try again ---

    ; Check AppData\Local\uclaw
    IfFileExists "$LOCALAPPDATA\uclaw\*.*" 0 _cu_localDone
      Sleep 3000
      RMDir /r "$LOCALAPPDATA\uclaw"
      IfFileExists "$LOCALAPPDATA\uclaw\*.*" 0 _cu_localDone
        nsExec::ExecToStack 'cmd.exe /c rd /s /q "$LOCALAPPDATA\uclaw"'
        Pop $0
        Pop $1
    _cu_localDone:

    ; Check AppData\Roaming\uclaw
    IfFileExists "$APPDATA\uclaw\*.*" 0 _cu_roamingDone
      Sleep 3000
      RMDir /r "$APPDATA\uclaw"
      IfFileExists "$APPDATA\uclaw\*.*" 0 _cu_roamingDone
        nsExec::ExecToStack 'cmd.exe /c rd /s /q "$APPDATA\uclaw"'
        Pop $0
        Pop $1
    _cu_roamingDone:
    ...
    StrCpy $R3 ""
    IfFileExists "$LOCALAPPDATA\uclaw\*.*" 0 +2
      StrCpy $R3 "$R3$\r$\n  • $LOCALAPPDATA\uclaw"
    IfFileExists "$APPDATA\uclaw\*.*" 0 +2
      StrCpy $R3 "$R3$\r$\n  • $APPDATA\uclaw"
  ...
  _cu_enumLoop:
    ...
    RMDir /r "$R3\AppData\Local\uclaw"
    RMDir /r "$R3\AppData\Roaming\uclaw"
