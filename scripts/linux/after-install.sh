...
# Create symbolic link for UClaw app binary
if [ -x /opt/UClaw/uclaw ]; then
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/UClaw/chrome-sandbox' || true
else
    chmod 0755 '/opt/UClaw/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    ...
    APPARMOR_PROFILE_TARGET='/etc/apparmor.d/uclaw'
