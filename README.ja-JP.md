...
UClawを初めて起動すると、**セットアップウィザード**が以下の手順をガイドします：

1. **言語と地域** – 使用する言語・地域の設定
2. **ワークスペース** – OpenClaw データの保存先を選択
3. **環境チェック** – バンドル済みランタイムと Gateway を検証
4. **AI設定** – 必要に応じて既定の New API エンドポイントとモデルを設定
5. **必須コンポーネント** – 既定のローカルツールをインストール

サポート対象のシステム言語がある場合、ウィザードはその言語を初期選択し、未対応の場合は英語にフォールバックします。
選択したワークスペースに `.openclaw/openclaw.json` が既に存在する場合、ウィザードは既存の OpenClaw 設定を再利用し、AI キー入力手順をスキップします。
