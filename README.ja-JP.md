...
<p align="center">
  <img src="src/assets/logo.svg" width="128" height="128" alt="UClaw Logo" />
</p>
...
<p align="center">
  <a href="#機能">機能</a> •
  <a href="#なぜuclawなのか">なぜUClawなのか</a> •
  <a href="#はじめに">はじめに</a> •
...
<p align="center">
  ...
  <img src="https://img.shields.io/github/downloads/DepartureZSH/UClaw/total?color=%23027DEB" alt="Downloads" />
...
**UClaw**は、強力なAIエージェントと日常のユーザーとの間のギャップを埋めます。[OpenClaw](https://github.com/OpenClaw)をベースに構築されており、コマンドラインによるAIオーケストレーションを、アクセスしやすく美しいデスクトップ体験に変換します。ターミナルは不要です。

ワークフローの自動化、AI搭載チャネルの管理、インテリジェントなタスクのスケジューリングなど、UClawはAIエージェントを効果的に活用するために必要なインターフェースを提供します。

UClawはベストプラクティスのモデルプロバイダーが事前設定されており、Windowsおよび多言語設定をネイティブにサポートしています。もちろん、**設定 → 詳細設定 → 開発者モード**から高度な設定を微調整することもできます。
...
エンドユーザー向けガイドは [user-guide.md](user-guide.md)、オフラインで開ける HTML マニュアルは [docs/user-manual.html](docs/user-manual.html) を参照してください。ページごとのフィールド、スクリーンショット、チャンネル設定の詳細、よくあるトラブルシューティングを補足しています。

---

## なぜUClawなのか

AIエージェントの構築にコマンドラインの習得は不要であるべきです。UClawはシンプルな哲学のもとに設計されました：**強力な技術には、あなたの時間を尊重するインターフェースがふさわしい。**
...
UClawは公式の**OpenClaw**コアを直接ベースに構築されています。別途インストールを必要とせず、アプリケーション内にランタイムを組み込むことで、シームレスな「バッテリー同梱」体験を提供します。
...
`@agent` で別のエージェントを選ぶと、UClaw はデフォルトエージェントを経由せず、そのエージェント自身の会話コンテキストへ直接切り替えます。各エージェントのワークスペースは既定で分離されていますが、より強い実行時分離は OpenClaw の sandbox 設定に依存します。
...
UClaw には Tencent 公式の個人 WeChat チャンネルプラグインも同梱されており、Channels ページからアプリ内 QR フローで直接 WeChat を連携できます。
...
UClaw はドキュメント処理スキル（`pdf`、`xlsx`、`docx`、`pptx`）もフル内容で同梱し、起動時に管理スキルディレクトリ（既定 `~/.openclaw/skills`）へ自動配備し、初回インストール時に既定で有効化します。追加の同梱スキル（`find-skills`、`self-improving-agent`、`tavily-search`、`brave-web-search`）も既定で有効化されますが、必要な API キーが未設定の場合は OpenClaw が実行時に設定エラーを表示します。
...
互換ゲートウェイで `/models` が認証以外の理由で使えない場合、UClaw は API キー検証時に軽量な `/chat/completions` または `/responses` プローブへ自動フォールバックします。

### 🌙 アダプティブテーマ
ライトモード、ダークモード、またはシステム同期テーマ。UClawはあなたの好みに自動的に適応します。

### 🚀 自動起動設定
**設定 → 通用** から **システム起動時に自動起動** を有効化すると、ログイン後に UClaw が自動的に起動します。

### 起動診断の設定
起動ロード画面と Gateway 起動フェーズのタイムアウトは `electron/utils/startup-config.ts` に集約されています。高度なビルドでは、永続化された設定オブジェクト（`startup.stepTimeouts` と `startup.gateway`）または環境変数でデフォルト値を上書きできます。よく使う環境変数は `UCLAW_STARTUP_TIMEOUT_REMOTE_CONFIG_SYNC_MS`、`UCLAW_STARTUP_TIMEOUT_GATEWAY_START_MS`、`UCLAW_GATEWAY_READY_WAIT_TIMEOUT_MS`、`UCLAW_GATEWAY_CONNECT_HANDSHAKE_TIMEOUT_MS`、`UCLAW_GATEWAY_CHALLENGE_TIMEOUT_MS` です。
...
[Releases](https://github.com/DepartureZSH/UClaw/releases)ページから、お使いのプラットフォーム向けの最新 ZIP リリースをダウンロードしてください。GitHub Releases では ZIP パッケージと対応する `.blockmap` ファイルのみを公開します。

#### ソースからビルド

```bash
# リポジトリをクローン
git clone https://github.com/DepartureZSH/UClaw.git
cd UClaw
...
現在の商用 ZIP ビルドは、メインウィンドウ内の起動ページから直接開始します。旧来の初回セットアップウィザードは通常のユーザーフローから外れました。

- ZIP ビルドでは、アプリ横の `data/` に UClaw データを保存し、OpenClaw 実行ファイルは `data/workspace` に固定されます。
- ポータブル ZIP 版ではアプリ内の自動インストール更新は使用しません。新しい ZIP をダウンロードし、UClaw を終了してからプログラムファイルを置き換えてください。同梱 OpenClaw ランタイムは UClaw と一緒に更新されます。
- AI と web search の設定は、管理された会社設定サービスから自動同期されます。
- パッケージ資格情報がない、または期限切れの場合のみ、UClaw は **Company Key** ページを表示します。一般ユーザーは運用担当から提供された会社キーを入力するだけで、ワークスペースや API エンドポイント/API キーを手動設定する必要はありません。
- 旧 `/setup` リンクは Company Key ページを開き、運用担当による初期化またはポータブルワークベンチの再バインドにのみ使用します。ワークスペース設定ウィザードは商用 ZIP の通常ユーザーフローには含まれません。

> Web search について: UClaw は既定で New API サービスを通じた Kimi/Moonshot 形式の web search を使用します。一般ユーザーはアプリ内のワンクリック設定または切り替えを使えばよく、独自エンドポイントの構築や OpenClaw 設定ファイルの手動編集は不要です。

### プロキシ設定

UClawには、Electron、OpenClaw Gateway、またはTelegramなどのチャネルがローカルプロキシクライアントを介してインターネットにアクセスする必要がある環境向けに、組み込みのプロキシ設定が含まれています。
...
- 高度なプロキシフィールドが空の場合、UClawは`プロキシサーバー`にフォールバックします。
- プロキシ設定を保存すると、Electronのネットワーク設定が即座に再適用され、ゲートウェイが自動的に再起動されます。
- UClawはTelegramが有効な場合、プロキシをOpenClawのTelegramチャネル設定にも同期します。
- UClawのプロキシが無効な状態では、Gatewayの通常再起動時に既存のTelegramチャネルプロキシ設定を保持します。
...
| ページ | 主なフィールドとボタン | よく使う操作 |
|--------|------------------------|--------------|
| Chat | 新規チャット、セッション一覧、現在の対象、更新、入力欄、添付、送信/停止、Gateway 状態 | 質問する、出典付き web search を依頼する、現在の Agent 接続を確認する |
| Models | AI モデルプロバイダー、モデル ID、モデル一覧取得、API キー状態、web-search セクション | New API 設定を確認し、利用可能モデルを更新し、検索モデルを検証する |
| Settings | テーマ、言語、自動起動、Gateway 状態、ポート、ログ、プロキシ、更新、About | 表示と言語を調整し、更新確認、ログを開く、プロキシを設定する |
| Channels | チャンネル追加、アカウント ID、既定アカウント、Agent 紐付け、診断、Gateway 再起動 | WeChat/Feishu/DingTalk などを接続し、返信する Agent を指定する |
| Cron | タスク名、実行時刻、プロンプト、実行 Agent、送信アカウント、受信先、オン/オフ | 日報、定期要約、定期検索、外部配信を作成する |
| Skills | スキル名、有効状態、ソースパス、設定/API キー | 同梱スキルを確認し、フォルダを開き、必要な設定を入力する |

### チャンネル設定フィールド

UClaw はチャンネルごとに複数アカウントを扱えます。手動入力するアカウント ID は英小文字、数字、アンダースコア、ハイフンのみ、最大 64 文字、先頭は英小文字または数字にしてください。WeChat や WhatsApp のような QR コード型チャンネルはプラグインフローで管理されるため、通常は資格情報を手入力しません。

| チャンネル | 接続方式 | 必要なフィールド |
|------------|----------|------------------|
| WeChat | QR コード | 手入力不要。QR 確認フローに従う |
| WhatsApp | QR コード | 手入力不要。スマートフォンの WhatsApp でスキャン |
| Telegram | Bot token | Bot token、許可するユーザー ID |
| Discord | Bot token | Bot token、サーバー ID、チャンネル ID（任意） |
| Feishu / Lark | アプリ資格情報 | App ID、App Secret |
| DingTalk | アプリ資格情報 | Client ID / AppKey、Client Secret / AppSecret |
| WeCom | アプリ資格情報 | Bot ID、App Secret |
| QQ Bot | アプリ資格情報 | App ID、Client Secret |

---

## アーキテクチャ

UClawは、**デュアルプロセス + Host API 統一アクセス**構成を採用しています。Renderer は単一クライアント抽象を呼び出し、プロトコル選択とライフサイクルは Main が管理します：
...
- UClaw は Electron アプリのため、**1つのアプリインスタンスでも複数プロセス（main/renderer/zygote/utility）が表示される**のが正常です。
...
- ウィンドウの閉じるボタン（`X`）は既定でトレイへ最小化する動作で、完全終了ではありません。完全終了する場合はトレイメニューの **Quit UClaw** を使用してください。
...
pnpm run package:portable:dual  # 推奨される二分割ポータブル構成を作成
```

ヘッドレス Linux では Electron テストに表示サーバーが必要です。`xvfb-run -a pnpm run test:e2e` を利用してください。

USB/外付けドライブ向けの構成では、[`docs/portable-usb.md`](docs/portable-usb.md) の二分割レイアウトを推奨します。macOS の `.app` は APFS に置き、Windows/Linux ビルドと共有 UClaw data/workspace は ExFAT に置きます。GitHub の Windows `.zip` には v2 ポータブルマーカーが含まれ、`UClaw.exe` を直接起動すると隣の `data/` ディレクトリを使用し、OpenClaw ワークスペースは `data/workspace` に固定され、ドライブ文字付きの絶対パスは保存されません。新しい data root は古い Roaming/UClaw アプリデータを自動インポートしません。業務用 USB パッケージでは provisioning 情報を含め、Gateway 起動前に管理された Laf エンドポイントから New API と web-search 設定を同期できます。macOS が App Translocation を報告する場合は [`docs/macos-external-app-troubleshooting.md`](docs/macos-external-app-troubleshooting.md) を参照してください。
...
コミュニティからのコントリビューションを歓迎します！バグ修正、新機能、ドキュメントの改善、翻訳など、あらゆる貢献がUClawをより良くするのに役立ちます。
...
UClawは優れたオープンソースプロジェクトの上に構築されています：

- [ClawX](https://github.com/ValueCell-ai/ClawX) – ValueCell-ai による元のオープンソースデスクトップアプリ系譜
...
UClaw パートナープログラムを開始します。特に、カスタム AI エージェントや自動化ニーズを持つより多くの顧客に UClaw を紹介してくださるパートナーを募集しています。

パートナーの皆さまには、見込みユーザーや案件との接点づくりを担っていただき、UClaw チームは技術サポート、カスタマイズ、統合を全面的に提供します。

AI ツールや自動化に関心のある顧客とお仕事をされている方は、ぜひご一緒できればうれしいです。

不具合報告、リリースに関する質問、再現手順付きの相談は [GitHub Issues](https://github.com/DepartureZSH/UClaw/issues) をご利用ください。
...
<p align="center">
  <img src="https://api.star-history.com/svg?repos=DepartureZSH/UClaw&type=Date" alt="スター履歴チャート" />
</p>
...
UClawは[MITライセンス](LICENSE)の下でリリースされています。本ソフトウェアの使用、変更、配布は自由に行えます。

---

<p align="center">
  <sub>DepartureZSH がメンテナンスしています</sub>
</p>
