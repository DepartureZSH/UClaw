...
### 起動診断の設定
起動ロード画面と Gateway 起動フェーズのタイムアウトは `electron/utils/startup-config.ts` に集約されています。高度なビルドでは、永続化された設定オブジェクト（`startup.stepTimeouts` と `startup.gateway`）または環境変数でデフォルト値を上書きできます。よく使う環境変数は `UCLAW_STARTUP_TIMEOUT_GATEWAY_START_MS`、`UCLAW_GATEWAY_READY_WAIT_TIMEOUT_MS`、`UCLAW_GATEWAY_CONNECT_HANDSHAKE_TIMEOUT_MS`、`UCLAW_GATEWAY_CHALLENGE_TIMEOUT_MS` です。
...
USB/外付けドライブ向けの構成では、[`docs/portable-usb.md`](docs/portable-usb.md) の二分割レイアウトを推奨します。macOS の `.app` は APFS に置き、Windows/Linux ビルドと共有 UClaw data/workspace は ExFAT に置きます。ランチャーは `--uclaw-data-root` を明示的に渡します。GitHub の Windows `.zip` にはポータブルマーカーも含まれるため、`UClaw.exe` を直接起動しても隣の `data/` ディレクトリを使用します。新しい data root は古い Roaming/UClaw アプリデータを自動インポートしません。macOS が App Translocation を報告する場合は [`docs/macos-external-app-troubleshooting.md`](docs/macos-external-app-troubleshooting.md) を参照してください。
