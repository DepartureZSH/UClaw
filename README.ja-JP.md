...
USB/外付けドライブ向けのポータブル構成では、[`docs/portable-usb.md`](docs/portable-usb.md) の二分割レイアウトを推奨します。macOS の `.app` は APFS に置き、Windows/Linux ビルドと共有 UClaw data/workspace は ExFAT に置きます。macOS が App Translocation を報告する場合は [`docs/macos-external-app-troubleshooting.md`](docs/macos-external-app-troubleshooting.md) を参照してください。
