...
import { getUClawProviderStore } from '../services/providers/store-instance';
import {
...
export async function storeApiKey(providerId: string, apiKey: string): Promise<boolean> {
  try {
    ...
    const s = await getUClawProviderStore();
 ...
 * Also synchronizes UClaw local provider list with OpenClaw's actual config.
 */
