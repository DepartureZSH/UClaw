...
/**
 * Merge a UClaw context section into an existing file's content.
 * If markers already exist, replaces the section in-place.
 * Otherwise appends it at the end.
 */
export function mergeUClawSection(existing: string, section: string): string {
  const wrapped = `${UCLAW_BEGIN}\n${section.trim()}\n${UCLAW_END}`;
  const beginIdx = existing.indexOf(UCLAW_BEGIN);
  const endIdx = existing.indexOf(UCLAW_END);
  if (beginIdx !== -1 && endIdx !== -1) {
    return existing.slice(0, beginIdx) + wrapped + existing.slice(endIdx + UCLAW_END.length);
  }
...
/**
 * Detect and remove bootstrap .md files that contain only UClaw markers
 * with no meaningful OpenClaw content outside them.
 */
export async function repairUClawOnlyBootstrapFiles(): Promise<void> {
  ...
  for (const workspaceDir of workspaceDirs) {
    ...
    for (const file of entries) {
      ...
      const beginIdx = content.indexOf(UCLAW_BEGIN);
      const endIdx = content.indexOf(UCLAW_END);
      if (beginIdx === -1 || endIdx === -1) continue;

      const before = content.slice(0, beginIdx).trim();
      const after = content.slice(endIdx + UCLAW_END.length).trim();
      if (before === '' && after === '') {
        try {
...
/**
 * Merge UClaw context snippets into workspace bootstrap files that
 * already exist on disk.  Returns the number of target files that were
 * skipped because they don't exist yet.
 */
async function mergeUClawContextOnce(): Promise<number> {
  ...
  let files: string[];
  try {
    files = (await readdir(contextDir)).filter((f) => f.endsWith('.uclaw.md'));
  } catch {
    return 0;
  }
  ...
  for (const workspaceDir of workspaceDirs) {
    ...
    for (const file of files) {
      const targetName = file.replace('.uclaw.md', '.md');
      ...
      const merged = mergeUClawSection(existing, section);
      if (merged !== existing) {
...
/**
 * Ensure UClaw context snippets are merged into the openclaw workspace
 * bootstrap files.
 */
export async function ensureUClawContext(): Promise<void> {
  let skipped = await mergeUClawContextOnce();
  if (skipped === 0) return;

  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    await new Promise((r) => setTimeout(r, RETRY_INTERVAL_MS));
    skipped = await mergeUClawContextOnce();
    if (skipped === 0) {
      logger.info(`UClaw context merge completed after ${attempt} retry(ies)`);
      return;
    }
    logger.debug(`UClaw context merge: ${skipped} file(s) still missing (retry ${attempt}/${MAX_RETRIES})`);
  }

  logger.warn(`UClaw context merge: ${skipped} file(s) still missing after ${MAX_RETRIES} retries`);
}
