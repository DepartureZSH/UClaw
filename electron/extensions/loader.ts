...
function resolveManifestPath(): string {
  if (app.isPackaged) {
    return join(process.resourcesPath, 'uclaw-extensions.json');
  }
  return join(app.getAppPath(), 'uclaw-extensions.json');
}
...
export async function loadExtensionsFromManifest(): Promise<void> {
  ...
  if (existsSync(manifestPath)) {
    ...
    logger.debug('[extensions] No uclaw-extensions.json found, loading all builtin extensions');
  }
