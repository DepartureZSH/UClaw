...
(function () {
  ...
  if (globalThis.__uclawFetchPatched) return;
  globalThis.__uclawFetchPatched = true;

  globalThis.fetch = function uclawFetch(input, init) {
    var url =
  ...
  if (process.platform === 'win32') {
    try {
      ...
      if (!cp.__uclawPatched) {
        cp.__uclawPatched = true;
