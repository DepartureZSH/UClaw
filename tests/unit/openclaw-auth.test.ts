...
const { testHome, testUserData } = vi.hoisted(() => {
  ...
  return {
    testHome: `/tmp/uclaw-openclaw-auth-${suffix}`,
    testUserData: `/tmp/uclaw-openclaw-auth-user-data-${suffix}`,
  };
});
...
// NOTE: syncWebSearchApiKeysFromEnv approach was superseded.
// The moonshot web-search plugin now gets its key via KIMI_API_KEY env var,
// which is aliased from the active provider's key in loadProviderEnv() (config-sync.ts).
// sanitizeOpenClawConfig() strips inline apiKey fields so the plugin falls back
// to readProviderEnvValue(["KIMI_API_KEY", "MOONSHOT_API_KEY"]) automatically.
